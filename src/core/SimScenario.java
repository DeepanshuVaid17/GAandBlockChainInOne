...
public class SimScenario implements Serializable {
	...
	protected void createHosts() {
		...
		Map<String,List<Coord>> coordsById = new HashMap<String,List<Coord>>();
		...
		for(int i = 1;i<nrofGroups;i++){
			...
			//Adding coords if Working Day Movement
			
			if(s.getSetting(MOVEMENT_MODEL_S).compareTo("WorkingDayMovement")==0){
				// creates prototypes of and MovementModel
				WorkingDayMovement wdm =
						(WorkingDayMovement)s.createIntializedObject(MM_PACKAGE +
								s.getSetting(MOVEMENT_MODEL_S));
				List<Coord> coords = new ArrayList<Coord>();
				coords.add(wdm.getHomeLocation());
				coords.add(wdm.getOfficeLocation());
				coords.add(wdm.getShoppingLocation());
				Coord tmp4 = new Coord(0,0);
				for(Coord coord:coords){
					double x = coord.getX()+tmp4.getX();
					double y = coord.getY()+tmp4.getY();
					tmp4.setLocation(x,y);
				}
				double x = tmp4.getX()/coords.size();
				double y = tmp4.getY()/coords.size();
				tmp4.setLocation(x, y);
				coords.add(tmp4);
				coordsById.put(gid, coords);
			}
		}
		for(int i = 1;i<=nrofGroups;i++){
			Settings s = new Settings(GROUP_NS+i);
			...
			if(s.getSetting(MOVEMENT_MODEL_S).compareTo("WorkingDayMovement")==0){
				...
				chromosome.add(gid);
				chromosome.add(gid);
				int sizeOfRemainingChromosome = 5;
				int indexOfAvgCoord = 3;
				Coord avgCoord = coordsById.get(gid).get(indexOfAvgCoord);
				List<Pair<Double,String>> nearestNeighbours = new ArrayList<Pair<Double,String>>();
				for (Map.Entry<String, List<Coord>> entry : coordsById.entrySet()) {
					if(entry.getKey()==gid)
						continue;
					Coord neighbourAvgCoord = entry.getValue().get(indexOfAvgCoord);
					double dist = avgCoord.distance(neighbourAvgCoord);
					nearestNeighbours.add(new Pair<Double,String>(dist,entry.getKey()));
				}
				//DOUBT WHETHER IT WORKS OR NOT
				Collections.sort(nearestNeighbours, new Comparator<Pair<Double, String>>() {
				    @Override
				...
				for(int j = 0;j<sizeOfRemainingChromosome;j++){
					chromosome.add(nearestNeighbours.get(j).getValue());
				}
				neighbours.put(gid, chromosome);
			}
		}
		//Deep----------
		for (int i=1; i<=nrofGroups; i++) {
			...
			if(s.getSetting(MOVEMENT_MODEL_S).compareTo("WorkingDayMovement")==0){
				...
				MovementModel Stationary =
						(MovementModel)s.createIntializedObject(MM_PACKAGE +"StationaryMovement");
				for(int j = 0;j<Math.min(10,wdm.getAllHomesLocation().size());j++){
					ModuleCommunicationBus comBus = new ModuleCommunicationBus();
					DTNHost host = new DTNHost(this.messageListeners,
						this.movementListeners,	gid, interfaces, comBus,
					...
					host.setNeigbours(neighbours);
					host.isWorkingDayMovement = true;
					host.setLocation(wdm.getAllHomesLocation().get(j));
					host.setIsItAStationary(1);
					hosts.add(host);
		
			    }
				for(int j = 0;j<Math.min(10,wdm.getAllOfficesLocation().size());j++){
					ModuleCommunicationBus comBus = new ModuleCommunicationBus();
					DTNHost host = new DTNHost(this.messageListeners,
						this.movementListeners,	gid, interfaces, comBus,
						Stationary, mRouterProto);
					...
					host.setNeigbours(neighbours);
					host.isWorkingDayMovement = true;
					host.setLocation(wdm.getAllOfficesLocation().get(j));
					host.setIsItAStationary(1);
					hosts.add(host);
